...
const screenForNames = (arr) => {
  // Solution code here...
  let regex = /((^Mr. [A-z])|(^Ms. [A-z])|(^Dr. [A-z])|(^Mrs. [A-z]))/;

  let newArrr = arr.filter((name) => {
...
const toTitleCase = (arr) => {
  ...
  let newArr = arr.map(
    (str) =>
      str.toLocaleUpperCase().split("").splice(0, 1).join("") +
      str.split("").splice(1).join("")
  );
...
let biggerThanLuke = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (Number(arr[i].mass) > Number(arr[0].mass)) {
